...
class BHead(nn.Module):
    def __init__(self, n_chs_in, n_chs_out):
        super().__init__()
        self.sa = scitex_nn.SpatialAttention(n_chs_in)
...
class BNet(nn.Module):
    def __init__(self, BNet_config, MNet_config):
        ...
        self.sc = scitex_nn.SwapChannels()
        self.dc = scitex_nn.DropoutChannels(dropout=0.01)
        self.fgc = scitex_nn.FreqGainChanger(
        ...
        self.cgcs = [
            scitex_nn.ChannelGainChanger(n_ch)
            for n_ch in BNet_config["n_chs_of_modalities"]
        ]
        # self.cgc = scitex_nn.ChannelGainChanger(N_VIRTUAL_CHS)

        # MNet_config["n_chs"] = BNet_config["n_virtual_chs"]  # BNet_config["n_chs"] # override

        n_chs = BNet_config["n_virtual_chs"]
        self.blk1 = scitex_nn.ResNetBasicBlock(n_chs, n_chs)
        self.blk2 = scitex_nn.ResNetBasicBlock(int(n_chs / 2**1), int(n_chs / 2**1))
        self.blk3 = scitex_nn.ResNetBasicBlock(int(n_chs / 2**2), int(n_chs / 2**2))
        self.blk4 = scitex_nn.ResNetBasicBlock(int(n_chs / 2**3), int(n_chs / 2**3))
        self.blk5 = scitex_nn.ResNetBasicBlock(1, 1)
        self.blk6 = scitex_nn.ResNetBasicBlock(1, 1)
        self.blk7 = scitex_nn.ResNetBasicBlock(1, 1)

        # self.MNet = scitex_nn.MNet_1000(MNet_config)
...
if __name__ == "__main__":
    ...
    # m = scitex_nn.ResNetBasicBlock(19, 19).cuda()
    # m(x_EEG)
