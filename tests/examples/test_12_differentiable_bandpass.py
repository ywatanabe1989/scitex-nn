"""Smoke test for examples/12_differentiable_bandpass.ipynb — runs jupyter nbconvert --execute."""
...
NOTEBOOK = (
    ...
    / "examples" / "12_differentiable_bandpass.ipynb"
)


@pytest.mark.skipif(shutil.which('jupyter') is None, reason='jupyter not installed')
def test_12_differentiable_bandpass_exists(tmp_path: Path) -> None:
    # Arrange
    # Act
    # Assert
...
@pytest.mark.skipif(shutil.which('jupyter') is None, reason='jupyter not installed')
def test_12_differentiable_bandpass_returncode(tmp_path: Path) -> None:
    # Arrange
    # Act
    # Assert
    pass
    ...
    shutil.copy(NOTEBOOK, target)
    r = subprocess.run(['jupyter', 'nbconvert', '--to', 'notebook', '--execute', '--output', target.name, str(target)], cwd=tmp_path, capture_output=True, text=True, timeout=300)
    assert r.returncode == 0, f'12_differentiable_bandpass.ipynb failed:\nSTDOUT:\n{r.stdout}\nSTDERR:\n{r.stderr}'
