...
This module also wires up subprocess coverage at import time so any
child Python interpreter spawned during the test run (`subprocess.run`,
`jupyter nbconvert --execute`, `pytest-xdist` workers, etc.) writes its
coverage shard alongside the parent's. See skill leaf
...
we therefore *force-set* both env vars.
"""
...
def _ensure_subprocess_coverage_shim() -> None:
    ...
    pth = purelib / "_scitex_nn_subprocess_coverage.pth"
    shim = (
        "import os, coverage\n"
